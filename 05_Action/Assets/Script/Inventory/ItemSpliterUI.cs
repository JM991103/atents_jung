...
public class ItemSpliterUI : MonoBehaviour, IScrollHandler
{
    ...
    /// <returns>true면 스크린 좌표가 아이템 분리기 안이다. false면 밖이다.</returns>
    private bool IsAreaInside(Vector2 screenPos)
    {
        RectTransform rectTransform = (RectTransform)transform;
        float halfWidth = rectTransform.rect.width * 0.5f;

        // 아이템 분리기 영역의 왼쪽 아래(min)와 오른쪽 위(max)를 계산하기. 아이템 분리기의 pivot이 아래쪽 가운데에 있어서 이렇게 계산
        ...
        // 결과는 위치가 min보다는 크도 max보다는 작을 때만 true.
