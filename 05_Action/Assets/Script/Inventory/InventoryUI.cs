...
public class InventoryUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        spliter.onOkClick += OnSplitOK;     // 스플리터가 가지고 있는 onOkClick 델리게이트 함수 등록
    ...
    public void InitializeInventory(Inventory playerInven)
    {
        ...
        if ( Inventory.Default_Inventory_Size != inven.SlotCount )  // 인벤토리 크기가 기본과 다를 때의 처리
        {
            ...
            float slotSideLenght = Mathf.Floor(Mathf.Sqrt(slotArea)) - grid.spacing.x;  // spacing 크기 고려해서 slot 한변의 길이 구하기
            grid.cellSize = new Vector2(slotSideLenght, slotSideLenght);                // 계산 결과 적용
        ...
        for (uint i = 0; i < inven.SlotCount; i++)
        {
            ...
            slotUIs[i].onClick += OnClick;                // 클릭을 했을때 실행될 함수 연결
    ...
    {
        inven.MoveItem(slotID, Inventory.TempSlotIndex);    // 슬롯에 있는 아이템을 임시 슬롯으로 모두 옮김
    ...
    /// </summary>
    /// <param name="obj">클릭된 슬롯의 ID</param>
    ...
    {
        ItemSlotUI target = slotUIs[slotID];
