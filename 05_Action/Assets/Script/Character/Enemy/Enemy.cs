...
public class Enemy : MonoBehaviour
{
    ...
    // 상태 관련 변수 ---------------------------------------------------------------------------------------------------
    EnemyState state;               // 현재 적의 상태(대기 상태인가, 순찰 상태인가)
    ...
    private void Start()
    {
        moveSpeedPerSecond = moveSpeed * Time.fixedDeltaTime;   // 자주 사용하는 연산 미리 계산해 놓기

        // wayPoints 가 없을 때를 대비한 코드
        if (wayPoints != null)
        {
    ...
    void Update_Patrol()
    {
        ...
        // 도착 확인
        if ((transform.position - moveTarget.position).sqrMagnitude < 0.01f)    // 도착 확인
        {
