    ...
    public override void OnInspectorGUI()
    {
        if (itemData != null)   // ItemData가 있는지 확인
        {
            if (itemData.itemIcon != null) // itemData에 itemIcon이 있는지 확인
            {
                ...
                texture = AssetPreview.GetAssetPreview(itemData.itemIcon);// itemData.itemIcon에서 texture 가져오기
                if (texture != null)    // 텍스텨가 있으면 
                {
                    GUILayout.Label("", GUILayout.Height(64), GUILayout.Width(64)); // 64 * 64 크기 영역을 잡고
