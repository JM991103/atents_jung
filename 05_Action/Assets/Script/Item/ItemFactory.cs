...
public class ItemFactory
{
    ...
    // overloading : 이름은 같은데 파라메터가 다른 함수를 만드는 것
    ...
    public static GameObject MakeItem(ItemIDCode code)
    {
        ...
        SphereCollider sc = obj.AddComponent<SphereCollider>();     // 콜라이더 추가
        sc.isTrigger = true;
        sc.radius = 0.5f;
    ...
    /// <summary>
    /// 코드를 이용해 특정 위치에 아이템을 한번에 어러개 생성하는 함수
    /// </summary>
    /// <param name="code">생성할 아이템의 아이템 코드</param>
    /// <param name="count">생성할 갯수</param>
    ...
    /// <summary>
    /// 아이텝 ID를 이용해 특정 위치에 아이템을 한번에 어러개 생성하는 함수
    /// </summary>
    /// <param name="code">생성할 아이템의 아이템 코드</param>
    /// <param name="count">생성할 갯수</param>
    /// <param name="position">생성할 기준 위치</param>
    ...
    public static GameObject[] MakeItem(int id, int count, Vector3 position, bool randomNoise = false)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            objs[i] = MakeItem(id, position, randomNoise); // count만큼 반복해서 아이템 생성
        }
        return objs;                                // 생성한 것 리턴
    }
}