...
public class RankPanel : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (var tab in tabs)
        {
            ...
            tab.onTabSelected += (newSelectedTab) =>
            {
                ...
                {
                    selectedTab.IsSelected = false; // 이전탭 끄기
        ...
        // 토글 버튼이 on/off될 때 실행될 델리게이트에 함다 함수 등록
        toggle.onToggleChange += (isOn) =>
        {
            if (isOn && selectedTab != null)    // 토글 버튼이 켜지고 선택된 탭이 있을 때
            {
                // 선택된 탭 체크 이유 : 토글 버튼의 Start가 RankPanel의 Start보다 먼저 실행되었을 때 문제
    ...
    private void Start()
    {
        // 게임 매니저에서 알려주는 타이밍에 따라 열고 닫기 설정
