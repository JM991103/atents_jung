...
//3. 데이터를 확신할 수 있다.
//4. Static 멤버를 이용해서 쉽게 접근 할 수 있도록 해준다.
...
public class Singleton<T> : MonoBehaviour where T : Component   
{
    ...
    public static T Inst
    {
        get
        {
            if(_instance == null)
            {
                ...
                if (obj != null)
                {
                    // 다른 객체가 있다. 그러면 있는 객체를 사용한다.
                    _instance = obj;
                }
                else
                {
                    // 다른 객체가 없다. 없으면 새로 만든다.
    ...
    /// <summary>
    /// 오브젝트가 생성 완료된 직후에 호출(씬에 싱글톤 오브젝트가 여러개 배치된 상황일 때 처리)
    /// </summary>
    private void Awake()
    {
        if (_instance == null)
        {
            // 처음 만들어진 싱글톤 게임 오브젝트
            _instance = this as T;      // _instace에 이 스크립트의 객체 저장
            DontDestroyOnLoad(this.gameObject); // 씬이 사라지더라도 게임 오브젝트를 삭제하지 않게 하는 코드

            SceneManager.sceneLoaded += OnSceneLoaded;  // 씬 로드가 완료되면 Initialize 
        }
        else
        {
