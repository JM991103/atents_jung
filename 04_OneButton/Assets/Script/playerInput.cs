...
public class playerInput : MonoBehaviour
{
    ...
    {
        player = new BirdInput();               // 인풋액션 객체 생성
        rigid = GetComponent<Rigidbody2D>();    //리지드바디 컴포넌트 찾기
    }

    private void start()    // 첫번째 update 함수가 실행되기 직전
    {
        isDead = false;                         // 우선 살아있다교 표시
    }

    private void OnEnable() // 오브젝트가 활성화 될 때
    {
        player.Bird.Enable();                   // 인풋액션 활성화
        player.Bird.Space.performed += OnJump;  // 점프 액션과 OnJump 함수 연결
    ...
    {
        player.Bird.Space.performed -= OnJump;  // 점프 액션과 OnJump 함수 연결 해제
        player.Bird.Disable();                  // 인풋액션 비활성화
    }
    ...
    private void FixedUpdate()  // 물리 업데이트 주기 마다(고정된 시간)
    {
        ...
        {
            float velocityY = Mathf.Clamp(rigid.velocity.y, -jumpPower, jumpPower);     // jumpPower만큼 최소/ 최댓값 지정
    ...
    void Die(ContactPoint2D contact)
    {
        ...
        Vector2 reflect = Vector2.Reflect(dir, contact.normal);                 // dir이 반사된벡터
        rigid.velocity = reflect * 10.0f + Vector2.left * 5.0f;                 // 반사되는 방향에 왼쪽으로 가는 힘 추가
        rigid.angularVelocity = 1000.0f;                                        // 회전 추가(초당 1000도 회전)
        ...
        if (!isDead)    
        {
            ...
            player.Bird.Disable();      // 죽었을 때 한번만 입력 막기
            isDead = true;              // 죽었다고 표시
