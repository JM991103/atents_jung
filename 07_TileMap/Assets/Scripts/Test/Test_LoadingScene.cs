...
public class Test_LoadingScene : MonoBehaviour
{
    ...
    /// </summary>
    public string nextScenName = "Test_Seemless_00";
    ...
    IEnumerator LoadScene()
    {
        ...
        loadRatio = 0.0f;
        async = SceneManager.LoadSceneAsync(nextScenName);  // 비동기 로딩 시작
        async.allowSceneActivation = false;                 // 로드 준비만 해놓은 설정

        while (loadRatio < 0.9f)
        {
            loadRatio = async.progress +0.1f;   // 비동기 로딩 진행 상황에 따라 loadRatio 증가(loadRatio가 1이 될 떄까지 반복)
        ...
        StopCoroutine(loadingTextCoroutine);        // loadingText 변경하는 코루틴을 중지
