...
public static class AStar 
{
    ...
    /// <param name="goal">도착 위치(그리드 좌표)</param>
    ...
    public static List<Vector2Int> PathFind(GridMap gridMap, Vector2Int start, Vector2Int goal)
    {
        ...
        if (gridMap.IsValidPosition(start) && gridMap.IsValidPosition(goal))
        {
            ...
            // A* 핵심 루틴 돌리기
            while(open.Count > 0)       // 오픈리스트가 비워질 때까지 계속 진행. (오픈 리스트가 비워지면 경로를 못찾은 것)
            {
                ...
                if(current != goal)     // 꺼낸 current가 도착지점인지 확인
                {
                    ...
                    // current 주변 8칸을 open리스트에 넣거나 이미 오픈리스트에 있으면 G값 갱신 시도
                    for(int y = -1; y < 2; y++ )
                    {
                        for (int x = -1; x < 2; x++)
                        {
                            ...
                            if (node == null)       // 맵 밖일 경우
                                continue;
                            if (node == current)    // current는 확인할 필요가 없음(자기 자신)
                                continue;
                            ...
                            {
                                // open 리스트에 없을 경우(G값의 초기값이 max니깐 무조건 들어옴)
                                if (node.parent == null)                // 처음사용하는 노드다 == open리스트에도 안들어가 있다.
                                {
            ...
            {
                // path를 완성하기 (current부터 시작해서 부모가 안나올때까지 쫒아가며 누적해준다.)
                path = new List<Vector2Int>();  // 리스트 생성
                Node result = current;          // current부터 시작해서
                while(result != null)           // result가 있는 한 계속 진행(다음 부모로 계속 진행했는데 result가 null이면 시작점이라는 뜻)
                {
    ...
    {
        return Mathf.Abs(current.x - goal.x) + Mathf.Abs(current.y - goal.y);   // 가로 세로로 직선 이동
    }
}
