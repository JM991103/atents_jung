...
public class Board : MonoBehaviour
{
    ...
    /// <summary>
    /// 특정 위치가 Board 안인지 밖인지 확인하는 함수
    /// </summary>
    /// <param name="gridPos">확인할 위치</param>
    /// <returns>true면 board안, false면 board 밖</returns>
    ...
    /// </summary>
    /// <param name="gridPos">확인할 위치</param>
    /// <returns>true면 공격이 실패한 지점. false면 성공한 지점</returns>
    public bool IsAttackFailPosition(Vector2Int gridPos)
    {
        int index = Board.GridToIndex(gridPos);
        // 공격을 한 지점은 상대방도 배가 있는지 없는지 알
        return bombInfo[index] && (shipInfo[index] == ShipType.None);   // 공격을 했고 거기에 함선이 없었다.
    }
    ...
    /// </summary>
    /// <param name="gridPos">확인할 위치</param>
    /// <returns>true면 공격이 성공한 지점. false면 실패한 지점</returns>
    ...
    public bool ShipDeplyment(Ship ship, Vector2Int pos)
    {
        ...
        bool result = IsShipDeployment(ship, pos, out gridPosition);    // 배치 가능여부 확인
        if (result)
        {
            // 배치가 가능하면
            foreach (var temp in gridPosition)
            {
            ...
            ship.Deploy(gridPosition);                      // 함선 배치 처리

            // 개발용 오브젝트 추가 부분
            if(shipDeploymentInfo != null)                  // 개발용 오브젝트를 만드는 클래스가 있으면 
            {
                Vector3[] worlds = new Vector3[gridPosition.Length];
                for (int i = 0; i < worlds.Length; i++)
                {
                    worlds[i] = GridToWorld(gridPosition[i]);       // 그리드 좌표를 모두 월드 좌표로 바꿔서
                }
    ...
    /// <summary>
    /// 특정 함선이 특정 위치에서 배치될 수 있는지 확인하는 함수
    /// </summary>
    /// <param name="ship">확인할 배(크기와 방향)</param>
    /// <param name="pos">확인할 배의 위치(함선의 머리 위치, 그리드 포지션)</param>
    /// <param name="gridPositions">함선이 배치될 수 있다고 확인된 위치(결과가 true일 때)</param>
    ...
    public bool IsShipDeployment(Ship ship, Vector2Int pos, out Vector2Int[] gridPositions)
    {
        ...
        switch (ship.Direction)         // 방향 결정 확인하기
        {
            case ShipDirection.North:
                dir = Vector2Int.up;    // (0,1) 북쪽을 바라보니까 꼬리로 갈수록 y는 증가
                break;
        ...
        {
            // 한칸이라도 보드를 벗어나거나 배가 배치되어 있으면 실패
    ...
    /// </summary>
    /// <param name="ship">확인할 배(크기와 방향)</param>
    /// <param name="gridPos">확인할 배의 위치(함선의 머리 위치, 그리드 포지션)</param>
    /// <returns>true면 배치가능, false면 배치 불가능</returns>
    ...
    /// </summary>
    /// <param name="ship">확인할 배(크기와 방향)</param>
    /// <param name="worldPos">확인할 배의 위치(함선의 머리 위치, 월드 좌표)</param>
    /// <returns>true면 배치가능, false면 배치 불가능</returns>
