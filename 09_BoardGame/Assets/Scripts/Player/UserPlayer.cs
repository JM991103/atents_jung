...
public class UserPlayer : PlayerBase
{
    ...
    Ship SelectedShip
    {
        ...
        set
        {
            if (selectedShip != value)
            {
                ...
                {
                    selectedShip.SetMaterialType();           // 머티리얼 타입을 normal로 돌리기
                ...
                {
                    selectedShip.SetMaterialType(false);      // 함선의 머리티얼 타입을 deploymode로 변경
    ...
    /// <summary>
    /// 클릭되었을 때 실행될 델리게이트들(게임 상태별로 각각 가지고 있음, null이면 해당 상태에서 입력받는 일이 없다는 것)
    /// </summary>
    Action<Vector2>[] onClick;

    /// <summary>
    /// 마우스가 움직였을 때 실행될 델리게이트들(게임 상태별로 각각 가지고 있음, null이면 해당 상태에서 입력받는 일이 없다는 것)
    /// </summary>
    Action<Vector2>[] onMouseMove;

    /// <summary>
    /// 마우스 휠이 돌아갈 때 실행될 델리게이트들(게임 상태별로 각각 가지고 있음, null이면 해당 상태에서 입력받는 일이 없다는 것)
    /// </summary>
    ...
    {
        Vector3 world = Camera.main.ScreenToWorldPoint(screenPos);              // 스크맂 좌표를 월드 좌표로 변경
    ...
    /// <summary>
    /// 함선 배치 씬에서 실행될 마우스 이동 처리 함수
    /// </summary>
    /// <param name="screenPos">마우스 커서가 있는 스크린 좌표</param>
    private void OnMouseMove_ShipDeployment(Vector2 screenPos)
    {
        if (SelectedShip != null && !SelectedShip.IsDeployed)               // 선택된 함선이 있고 아직 배치가 안됐으면
        {
