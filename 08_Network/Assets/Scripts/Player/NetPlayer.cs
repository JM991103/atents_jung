...
public class NetPlayer : NetworkBehaviour
{
    ...
    {
        GameObject ball = Instantiate(ballPrefab);           // 서버에서 만들기
    ...
    /// <param name="newValue">현재 값</param>
    private void OnAnimStateChange(PlayerAnimState previousValue, PlayerAnimState newValue)
    {
        anim.SetTrigger($"{newValue}"); // enum 값을 토대로 트리거 실행
    }

    /// <summary>
    /// networkEffectState가 변경되면 이펙트를 보여주거나 끄는 함수
    /// </summary>
    ...
    public void UpdateClientMoveAndRotateServerRpc(Vector3 moveDelta, float rotateDelta)
    {
        ...
        // NetworkVariable의 기본설정이 서버만 쓰기 권한을 가지고 있기 때문이다.
        networkMoveDelta.Value = moveDelta;
        networkRotateDelta.Value = rotateDelta;
    }

    [ServerRpc]
    void updataNetPlayerAnimStateServerRpc(PlayerAnimState playerAnimState)
    {
        // 값 변경하기
