...
public class NetworkTestController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            if (NetworkManager.Singleton.StartHost())   // 호스트로 시작하기
            {
                GameManager.Inst.Logger.Log("<#00ff00>호스트</color>가 시작되었습니다.");        // 시작에 성공
            }
            else
            {
                GameManager.Inst.Logger.Log("호스트 시작에 실패했습니다.");     // 시작에 실패
            }
        });

        starClient.onClick.AddListener(() =>            // 클라이언트 시작 버튼이 클릭 되었을 때
        {   
            if (NetworkManager.Singleton.StartClient()) // 클라이언트로 시작하기(시도)
            {
                GameManager.Inst.Logger.Log("클라이언트의 연결이 시작되었습니다."); // 시작에 성공
            }
            else
            {
